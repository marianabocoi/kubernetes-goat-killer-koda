...
kubectl apply -f kubernetes-goat/scenarios/cache-store/deployment.yaml
sleep 15
...
echo 'export POD_NAME=$(kubectl get pods --namespace secure-middleware -l "app=cache-store" -o jsonpath="{.items[0].metadata.name}")'> startup.sh
