...
kubectl apply -f kubernetes-goat/scenarios/kube-bench-security/node-job.yaml
kubectl apply -f kubernetes-goat/scenarios/kube-bench-security/master-job.yaml
...
echo 'export POD_NAME=$(kubectl get pods --namespace default -l "app=docker-bench-security" -o jsonpath="{.items[0].metadata.name}")'> startup.sh
